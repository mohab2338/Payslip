...
## 9. Add a web version for iPhone (no Apple Developer account needed)

A real iOS app needs Apple code signing (a paid Developer account). If you
don't have one, the practical workaround is to also build this same app as a
**website**. The iPhone user opens it in Safari — no App Store, no signing —
and because it talks to the same Firestore database, whatever either person
adds shows up for the other in real time.

> **Important — how the two people share data:** this app's data is scoped
> per Firebase account (the Firestore rules from step 2 only let a signed-in
> user see their *own* `uid`'s data). So for both people to see the same
> salary/purchases, **they both need to sign in with the same email and
> password** — on the Android app and in the web browser. There's no
> separate "family sharing" feature; it's literally the same account used
> from two devices, the same way you might share one Netflix login.

### 9a. Register a Web app in Firebase

1. Firebase console → **Project settings** (gear icon) → scroll to **Your
   apps** → click the **</>  (Web)** icon.
2. Give it a nickname (anything) → **Register app**. You do *not* need to
   check "Also set up Firebase Hosting."
3. Firebase shows you a config snippet like this:
   ```js
   const firebaseConfig = {
     apiKey: "AIzaSyXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",
     authDomain: "salary-tracker-xxxxx.firebaseapp.com",
     projectId: "salary-tracker-xxxxx",
     storageBucket: "salary-tracker-xxxxx.firebasestorage.app",
     messagingSenderId: "123456789012",
     appId: "1:123456789012:web:abcd1234efgh5678",
     measurementId: "G-XXXXXXXXXX"  // may not appear - that's fine
   };
   ```
   Click **Continue to console** once you've noted these values (you can
   always find them again later under Project settings → Your apps → this
   web app → the "SDK setup and configuration" section).

### 9b. Fill in the `web` block of `firebase_options.dart`

Open `lib/firebase_options.dart` and fill in the `web` block (near the
bottom of the file) using the values from the snippet above:

| In `firebase_options.dart` (`web` block) | Comes from the config snippet |
|---|---|
| `apiKey`             | `apiKey`             |
| `appId`              | `appId`              |
...
| `measurementId`      | `measurementId` (if Firebase didn't show you one, just delete that whole line) |

This is the same file you already edited for Android — you're just filling
in one more block in it, then committing/pushing again.

### 9c. Turn on GitHub Pages (one click, no CLI)

In your GitHub repo: **Settings → Pages** → under "Build and deployment,"
set **Source** to **GitHub Actions**. That's the entire setup — the workflow
below handles the rest.

### 9d. Push and let it build

`.github/workflows/web.yml` (included in this project) builds the web app
and deploys it automatically on every push to `main`, the same way the
Android workflow does. Watch it in the **Actions** tab.

Once it finishes, your site is live at:
```
https://<your-github-username>.github.io/<your-repo-name>/
```
(GitHub also shows this exact URL on the **Settings → Pages** screen, and
in the workflow run's "deploy" job output.)

### 9e. Using it on iPhone

1. Open that URL in Safari on the iPhone.
2. Log in with the **same email and password** used on the Android app (see
   the important note above).
3. Optional but recommended: tap the **Share** icon → **Add to Home
   Screen**. This gives it an app icon and launches full-screen, without a
   browser address bar — it feels like a real app.

### Known differences on web vs. the Android app

- **No fingerprint login.** Fingerprint/Face ID unlock only works in the
  native mobile app; the web version always asks for the account password.
  This is handled gracefully in the code — the fingerprint option simply
  won't appear on web, nothing breaks.
- **Exporting a month's CSV** works differently: on Android it opens the
  share sheet; on web it triggers a normal browser file download (into
  Safari's Downloads). Same CSV content either way.
...
- **Web build fails at "Fail early... web app was never registered"** — you
  skipped section 9b; fill in the `web` block of `firebase_options.dart`.
- **Web page loads but is blank / shows an error about Firebase** — usually
  means one of the `web` block values (especially `authDomain` or
  `projectId`) was mistyped. Re-check them against the Firebase console
  snippet in section 9a.
- **GitHub Pages shows a 404** — double check **Settings → Pages → Source**
  is set to **GitHub Actions** (not "Deploy from a branch"), and that the
  "Build & Deploy Web App" workflow run finished successfully in the
  **Actions** tab.
